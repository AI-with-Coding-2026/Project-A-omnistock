...
class MiddlewareRouteProtectionRegressionTests(TestCase):
    """Regression coverage for protected routes introduced by Task 2297."""
    ...
    def setUp(self):
        ...
        self.inventory_manager = User.objects.create_user(
            username='route_inventory_manager',
            password='testpass123',
            role=User.ROLE_INVENTORY_MANAGER,
        )
        self.sales_rep = User.objects.create_user(
            username='route_sales_rep',
            password='testpass123',
            role=User.ROLE_SALES_REP,
        )

    def test_anonymous_users_are_redirected_from_protected_routes(self):
        for route_name in ('order_create', 'product_create', 'supplier_create', 'reports'):
            with self.subTest(route=route_name):
                url = reverse(route_name)
                response = self.client.get(url)
                self.assertRedirects(response, f'{reverse("login")}?next={url}')

    def test_sales_rep_cannot_access_administrative_routes(self):
        self.client.force_login(self.sales_rep)

        for route_name in ('product_create', 'supplier_create', 'reports'):
            with self.subTest(route=route_name):
                self.assertEqual(self.client.get(reverse(route_name)).status_code, 403)

    def test_inventory_manager_can_manage_suppliers_but_not_products(self):
