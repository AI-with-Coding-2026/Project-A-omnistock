...
class OrderCreateStockTests(TestCase):
    ...
    def test_order_form_exposes_stock_for_client_side_quantity_validation(self):
        response = self.client.get(reverse('order_create'))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'stockMap')
        self.assertContains(response, 'hasInsufficientStock()')
        self.assertContains(
            response,
            'Requested quantity exceeds the available stock for one or more products.',
        )
