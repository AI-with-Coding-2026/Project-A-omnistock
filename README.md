...
Static assets (including the OmniStock logo) are served through Whitenoise using a hashed manifest for cache-busting. Build that manifest locally with:
...
- `users` **(**`core`**)** — authentication, custom user model/roles, public landing page, shared templates.
...
External requirement: **MySQL 8.x** for local development (see [Install MySQL](#2-install-mysql) above). Production deployments may target PostgreSQL via `dj-database-url` and `psycopg2-binary`; served through `gunicorn` with static files handled by `whitenoise`.
